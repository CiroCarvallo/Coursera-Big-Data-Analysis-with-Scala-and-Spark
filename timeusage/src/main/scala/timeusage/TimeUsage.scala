...
object TimeUsage extends TimeUsageInterface {
  ...
  def timeUsageByLifePeriod(): Unit = {
    ...
    //val pruebaDf = timeUsageGroupedSql(summaryDf)
  ...
  /** @return An RDD Row compatible with the schema produced by `dfSchema`
    ...
    */
  def row(line: List[String]): Row = Row.fromSeq(line.head.toString :: line.tail.map(s => s.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val columnsPN = columnNames.filter(col => primaryNeeds.exists(pn => col.startsWith(pn))).map(column)
    val columnsWA  = columnNames.filter(col => workingActivities.exists(pn => col.startsWith(pn))).map(column)
    val columnsL  = columnNames.filter(col => leisure.exists(pn => col.startsWith(pn)) &&
      !primaryNeeds.exists(pn => col.startsWith(pn)) && !workingActivities.exists(pn => col.startsWith(pn))).map(column)
    (columnsPN,columnsWA,columnsL)
  }
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed.groupBy("working","sex","age").agg(round(avg($"primaryNeeds"),1).as("primaryNeeds"),
      round(avg($"work"),1).as("work"),round(avg($"other"),1).as("other")).orderBy("working","sex","age")
  }
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    ...
    "SELECT working,sex,age,ROUND(AVG(primaryNeeds),1) as primaryNeeds,ROUND(AVG(work),1) as work,ROUND(AVG(other),1) " +
      "as other from "+ viewName +
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(r=> (r.working,r.sex,r.age)).agg(round(typed.avg[TimeUsageRow](_.primaryNeeds), 1)
      .as[Double].name("primaryNeeds"),
      round(typed.avg[TimeUsageRow](_.work), 1).as[Double].name("work"),
      round(typed.avg[TimeUsageRow](_.other), 1).as[Double].name("other"))
      .map(line => TimeUsageRow(line._1._1, line._1._2, line._1._3, line._2, line._3, line._4))
